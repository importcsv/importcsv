...
class DestinationCreate(BaseModel):
    ...
    destination_type: str = Field(default="supabase", pattern="^(supabase|webhook)$")
    ...
    def validate_config(self):
        ...
        if self.destination_type == "supabase":
            if not self.integration_id:
                raise ValueError("Supabase destination requires integration_id")
        elif self.destination_type == "webhook":
            if not self.webhook_url:
                raise ValueError("Webhook destination requires webhook_url")
...
class DestinationResponse(BaseModel):
    ...
    # Cloud-only: signing secret (not stored, fetched from Svix)
