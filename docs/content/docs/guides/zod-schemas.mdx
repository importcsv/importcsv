---
title: Zod Schemas
description: Type-safe CSV validation with automatic TypeScript inference
---

<Callout type="warn" title="Feature Status: Coming in Phase 1">
Direct `schema` prop support is planned for Phase 1 implementation. This guide shows the future API to help you prepare your schemas.

**For now**, use the `columns` array with validators (see [Migration Guide](#migration-guide) below).
</Callout>

## Why Zod?

Zod provides runtime validation with automatic TypeScript type inference, making your CSV imports type-safe from upload to database.

**Benefits:**
- ✅ **Automatic TypeScript inference** - No manual type definitions needed
- ✅ **Runtime + compile-time validation** - Catch errors before and during execution
- ✅ **Better error messages** - Clear, actionable validation errors
- ✅ **Ecosystem integration** - Works with tRPC, React Hook Form, Prisma, etc.
- ✅ **Schema composition** - Reuse and compose schemas easily
...
Replace the legacy `columns` array with a Zod schema:

### Before (Legacy Columns)
...
<CSVImporter
  ...
  onComplete={(data: User[]) => {
    ...
    data.forEach(user => {
      ...
      console.log(user.age); // TypeScript knows this is optional
    });
  }}
/>
```
...
<Cards>
  <Card title="🎨 Headless Components" href="/headless" description="Use schemas with headless primitives" />
  <Card title="📖 API Reference" href="/api" description="Complete props documentation" />
  <Card title="💡 Examples" href="/examples/basic" description="More real-world examples" />
  <Card title="🚀 Quickstarts" href="/getting-started" description="Framework-specific guides" />
</Cards>
